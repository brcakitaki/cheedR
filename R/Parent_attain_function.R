...
Parent_attain <- function(path) {
  ...
  if ("parent_highest.Educ.Attainment.Code" %in% colnames(x) == F) {
  ...
  x <- x %>%
    ...
    dplyr::mutate(Parent.attainment = min(Ed.hierarchy)) %>%
    dplyr::mutate(Parent.attainment = case_when(
      Parent.attainment == 1 ~ "Postgraduate",
      Parent.attainment == 2 ~ "Bachelor",
      Parent.attainment == 3 ~ "No HE"
    )) %>%
    dplyr::ungroup() %>%
    dplyr::distinct(Student.ID,Parent.attainment)
}
